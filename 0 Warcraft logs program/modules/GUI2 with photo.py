...
def plot_scatter():
    ...
    if filtered_df.empty:
        ...
        plt.imshow(img, extent=[df["X coord"].min(), df["X coord"].max(), df["Y coord"].min(), df["Y coord"].max()])
        
        plt.scatter(filtered_df["X coord"], filtered_df["Y coord"], alpha=0.6, color='blue')
        plt.title(f"Scatter Plot - {event_type} ({spell_name if spell_name else 'All Spells'})")
        plt.xlabel("X coord")
        plt.ylabel("Y coord")
        plt.grid()
        plt.show()
...
def plot_heatmap():
    ...
    if filtered_df.empty:
        ...
        sns.kdeplot(x=filtered_df["X coord"], y=filtered_df["Y coord"], cmap="Reds", fill=True, alpha=0.6)
        plt.title(f"Heatmap - {event_type} ({spell_name if spell_name else 'All Spells'})")
        plt.xlabel("X coord")
        plt.ylabel("Y coord")
        plt.grid()
        plt.show()
